...
import unittest
import string
from hangman import is_word_guessed, get_guessed_word, get_available_letters, match_with_gaps, show_possible_matches, get_score, penalize_wrong_guess
...
class TestHangman(unittest.TestCase):
    ...
    def test_get_available_letters(self):
        ...
        self.assertEqual(get_available_letters(
            ['a', 'd']), 'bcefghijklmnopqrstuvwxyz')
    ...
    def test_show_possible_matches(self):
        self.assertListEqual(show_possible_matches(
            "t__t"), "tact tart taut teat tent test text that tilt tint toot tort tout trot tuft twit".split())
    ...
    def test_penalize_wrong_guess(self):
        game_state_vowel = {"user_input": "a", "guess_left": 2}
        penalize_wrong_guess(game_state_vowel)
        self.assertEqual(game_state_vowel["guess_left"], 0)

        game_state_consonant = {"user_input": "b", "guess_left": 2}
        penalize_wrong_guess(game_state_consonant)
        self.assertEqual(game_state_consonant["guess_left"], 1)
